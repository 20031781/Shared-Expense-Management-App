...
2. Abilita/disabilita i toggle per:
   ...
   - **Esito validazione**
   - **Rimborsi**
3. Le preferenze vengono salvate tramite `PUT /api/auth/profile` e sono applicate a tutte le notifiche successive inviate dal backend.
...
## Endpoint di test
Gli endpoint di test richiedono autenticazione JWT e permettono di simulare ogni tipologia di notifica senza dover riprodurre l'intero flusso applicativo.
...
In caso di errori controlla che gli `id` esistano nel database e che gli utenti destinatari abbiano almeno un device token registrato.
